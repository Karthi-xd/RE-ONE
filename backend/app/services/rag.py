import re
import ollama
from ..core.config import settings, get_chroma_client


_collection_cache: dict[int, object] = {}


def _get_collection(year: int):
    if year not in _collection_cache:
        _collection_cache[year] = get_chroma_client().get_collection(name=f"events_{year}")
    return _collection_cache[year]
...
# Questions about the character itself, not about the year's archive.
# These used to fall through to "grounded" mode, where retrieval pulled
# back whatever chunks happened to be nearest by embedding distance - so
# "whats your name" and "what's your name" (different enough as raw text
# to retrieve different chunks) could get two different answers, one of
# which stumbled onto something usable and one of which didn't. An
# identity question should never depend on what the archive happens to
# contain, so it's routed the same way as small talk: straight to the
# persona, no retrieval, no "never heard of that" framing.
_IDENTITY_RE = re.compile(
    r"(what'?s your name|whats your name|who are you|who r u|"
    r"do you have a name|what should i call you|what do (?:i|we) call you|"
    r"how old are you|what grade are you|what'?s your age|"
    r"are you (?:a |an )?(?:ai|robot|bot|chatbot|computer|human|real person)|"
    r"are you real)",
    re.IGNORECASE,
)
...
def classify_query(year: int, query: str) -> tuple[str, int | None]:
    ...
    mode is one of:
      "chat"        - small talk / greeting / identity question, no
                      archive lookup needed
      "out_of_range" - the question names a year other than the one open
      "grounded"    - a real question about this year, use the archive
    """
    q = query.strip()

    if not q or _GREETING_RE.match(q) or _IDENTITY_RE.search(q):
        return "chat", None
